...
//TODO: Clean this up and start implementing Advantage Kit
public class Shoulder_Old extends SubsystemBase implements StateBasedSubsystem{
  ...
  private Directions mDirection = IDLE;

  private SubsystemStates subsystemState = HOLDING_POS;
  /** Creates a new Shoulder_Subsystem. */
  public Shoulder_Old() {
    m_motor = new CANSparkMax(Constants.ArmConstants.kShoulder_Id, MotorType.kBrushless);

    m_motor.restoreFactoryDefaults();
    m_motor.setSmartCurrentLimit(kShoulderCurrentLimit);
    m_motor.setInverted(kShoulderIsInverted);
    m_motor.setIdleMode(kShoulderIdleMode);
    m_motor.setSoftLimit(kForward, kShoulderForwardSoftLimit);
    m_motor.setSoftLimit(kReverse, kShoulderReverseSoftLimit);
    m_motor.enableSoftLimit(kForward, kShoulderSofLimitEnabled);
    m_motor.enableSoftLimit(kReverse, kShoulderSofLimitEnabled);
    m_motor.enableVoltageCompensation(kShoulderVoltageCompensation);
  ...
  public void updateDirection() {
    ...
    mDirection = m_motor.get() > 0 ?  UPWARDS : DOWNWARDS;
  }
  ...
  public void periodic() {
    ...
    //so the break will open before the motor powers up
    //doubts: visit: https://docs.wpilib.org/en/stable/docs/software/commandbased/command-scheduler.html
    updateDirection();
    automaticBreak();
  }
